...
# The engine's lsw_main.c includes "config.h", normally produced by autotools
# ./configure. It only needs the file to exist (it doesn't use the PACKAGE_*
# macros, and DECTALK_INSTALL_PREFIX has a fallback in dectalkf.h), so generate a
# minimal one — this lets a fresh checkout build without running ./configure.
ensure_config_h() {
  local cfg="$ROOT_DIR/upstream/src/config.h"
  [ -f "$cfg" ] && return
  cat > "$cfg" <<'EOF'
/* Minimal config.h for the Apple xcframework build (engine/build-xcframework.sh).
   Normally generated by autotools ./configure; only needs to be present. */
#define HAVE_ICONV 1
#define PACKAGE_NAME "DECtalk"
#define PACKAGE_TARNAME "dectalk"
#define PACKAGE_VERSION "apple"
#define PACKAGE_STRING "DECtalk apple"
